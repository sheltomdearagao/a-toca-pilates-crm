...
interface AddExpenseModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddExpense: (formData: { description: string; amount: number; dueDate: string; category: ExpenseCategory }) => void;
}
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Adicionar Nova Despesa">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
            ...
            <select name="category" value={formData.category} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-brand-primary focus:border-brand-primary text-gray-900">
        ...
        <div className="flex justify-end pt-4">
          ...
          <button type="submit" className="px-4 py-2 bg-brand-primary text-white rounded-md hover:bg-blue-800">Adicionar Despesa</button>
        </div>
      </form>
    </Modal>
  );
